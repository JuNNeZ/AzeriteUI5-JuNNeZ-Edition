...
function oUF:DisableBlizzard(unit)
	...
	local isSecretMode = (type(issecretvalue) == 'function')
	...
	elseif(unit:match('arena%d?$')) then
		if(isSecretMode) then return end
		if(not isArenaHooked) then
			isArenaHooked = true

			handleFrame(CompactArenaFrame)

			if(CompactArenaFrame and type(CompactArenaFrame.memberUnitFrames) == "table") then
