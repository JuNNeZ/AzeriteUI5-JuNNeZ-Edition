...
local function initObject(unit, style, styleFunc, header, ...)
	...
	for i = 1, num do
		...
		local objectUnit = object:GetAttribute('oUF-guessUnit') or object:GetAttribute('unit') or unit
		local suffix = object:GetAttribute('unitsuffix')

		-- Handle the case where someone has modified the unitsuffix attribute in
		-- oUF-initialConfigFunction.
		if(suffix and type(objectUnit) == 'string' and not objectUnit:match(suffix)) then
		...
		if(type(objectUnit) == 'string' and objectUnit ~= '' and not isEventlessUnit(objectUnit)) then
