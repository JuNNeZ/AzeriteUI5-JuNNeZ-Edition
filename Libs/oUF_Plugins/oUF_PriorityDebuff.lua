...
-- GetSpellInfo compatibility wrapper (moved to C_Spell.GetSpellInfo in modern WoW)
local GetSpellInfo
if C_Spell and C_Spell.GetSpellInfo then
	-- Modern WoW (Dragonflight+): C_Spell.GetSpellInfo returns a table
...
elseif _G.GetSpellInfo then
	-- Legacy WoW (Legion Remix, older Retail): GetSpellInfo is a global function
	GetSpellInfo = _G.GetSpellInfo
else
	-- Fallback: create a no-op function to prevent errors
...
local function Update(self, event, unit)
	if(type(unit) ~= "string" or (IsSecret and IsSecret(unit)) or unit ~= self.unit) then return end

	local element = self.PriorityDebuff

	-- WoW12 secret-value guard: UnitAura/FrameXML AuraUtil can throw when aura payloads are secret.
	-- Read auras behind pcall and treat inaccessible payloads as absent for this element.
	local function SafeUnitAuraHarmful(unitToken, index)
		local ok, _name, _icon, _count, _debuffType, _duration, _expirationTime, _unitCaster, _, _, _spellID, _, _isBossDebuff = pcall(UnitAura, unitToken, index, 'HARMFUL')
		if (not ok) then
			return nil
		end
		if (IsSecret and (_name and IsSecret(_name) or _spellID and IsSecret(_spellID) or _debuffType and IsSecret(_debuffType))) then
			return nil
		end
		return _name, _icon, _count, _debuffType, _duration, _expirationTime, _unitCaster, _spellID, _isBossDebuff
	end
	...
	if (IsSecret and IsSecret(canAttack)) then canAttack = false end
	if (IsSecret and IsSecret(isCharmed)) then isCharmed = false end
	local canDispel = not canAttack and not isCharmed
	...
	* event 	- the event which caused the update (string)
	...
	for i = 1,40 do
		local _name, _icon, _count, _debuffType, _duration, _expirationTime, _unitCaster, _spellID, _isBossDebuff = SafeUnitAuraHarmful(unit, i)

		if (not _name) then break end

		local isBlacklisted = self.blacklistedDebuffs and self.blacklistedDebuffs[_spellID]
...
local function UpdateDispelTypes(self, event, unit, ...)
	if(event == 'UNIT_SPELLCAST_SUCCEEDED') then
		...
		if(type(castUnit) ~= "string" or (IsSecret and IsSecret(castUnit)) or castUnit ~= 'player') then return end
